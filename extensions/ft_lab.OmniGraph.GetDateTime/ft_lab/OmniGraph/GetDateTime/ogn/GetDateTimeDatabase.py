...
class GetDateTimeDatabase(og.Database):
    ...
    INTERFACE = og.Database._get_interface([
        ('outputs:year', 'int', 0, 'Year', 'output year', {ogn.MetadataKeys.DEFAULT: '2000'}, True, 0, False, ''),
        ('outputs:month', 'int', 0, 'Month', 'output month', {ogn.MetadataKeys.DEFAULT: '1'}, True, 0, False, ''),
        ('outputs:day', 'int', 0, 'Day', 'output day', {ogn.MetadataKeys.DEFAULT: '1'}, True, 0, False, ''),
        ('outputs:hour', 'int', 0, 'Hour', 'output hour', {ogn.MetadataKeys.DEFAULT: '1'}, True, 0, False, ''),
        ('outputs:minute', 'int', 0, 'Minute', 'output minute', {ogn.MetadataKeys.DEFAULT: '0'}, True, 0, False, ''),
        ('outputs:second', 'int', 0, 'Second', 'output second', {ogn.MetadataKeys.DEFAULT: '0'}, True, 0, False, ''),
    ])
    ...
    class ValuesForOutputs(og.DynamicAttributeAccess):
        LOCAL_PROPERTY_NAMES = { "year", "month", "day", "hour", "month", "second" }
        ...
        @property
        def year(self):
            value = self._batchedWriteValues.get(self._attributes.year)
            if value:
                return value
            else:
                data_view = og.AttributeValueHelper(self._attributes.year)
                return data_view.get()

        @year.setter
        def year(self, value):
            self._batchedWriteValues[self._attributes.year] = value

        @property
        def month(self):
            value = self._batchedWriteValues.get(self._attributes.month)
            if value:
                return value
            else:
                data_view = og.AttributeValueHelper(self._attributes.month)
                return data_view.get()

        @month.setter
        def month(self, value):
            self._batchedWriteValues[self._attributes.month] = value

        @property
        def day(self):
            value = self._batchedWriteValues.get(self._attributes.day)
            if value:
                return value
            else:
                data_view = og.AttributeValueHelper(self._attributes.day)
                return data_view.get()

        @day.setter
        def day(self, value):
            self._batchedWriteValues[self._attributes.day] = value

        @property
        def hour(self):
            value = self._batchedWriteValues.get(self._attributes.hour)
            if value:
                return value
            else:
                data_view = og.AttributeValueHelper(self._attributes.hour)
                return data_view.get()

        @hour.setter
        def hour(self, value):
            self._batchedWriteValues[self._attributes.hour] = value

        @property
        def minute(self):
            value = self._batchedWriteValues.get(self._attributes.minute)
            if value:
                return value
            else:
                data_view = og.AttributeValueHelper(self._attributes.minute)
                return data_view.get()

        @minute.setter
        def minute(self, value):
            self._batchedWriteValues[self._attributes.minute] = value

        @property
        def second(self):
            value = self._batchedWriteValues.get(self._attributes.second)
            if value:
                return value
            else:
                data_view = og.AttributeValueHelper(self._attributes.second)
                return data_view.get()

        @second.setter
        def second(self, value):
            self._batchedWriteValues[self._attributes.second] = value
